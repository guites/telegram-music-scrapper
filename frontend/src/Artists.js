...
export const Artists = () => {
	...
	const setSelectedSuggestionWithPosition = async (suggestion) => {
		// pre load the latitude and longitude for the received suggestion
		const suggestionWithPosition = await getArtistPositions(suggestion)
		setSelectedSuggestion(suggestionWithPosition);
	}
	...
	const getMusicBrainzSuggestions = async artist => {
		...
		if (suggestions.length > 0) {
			setSelectedSuggestionWithPosition(suggestions[0]);
		} else {
							...
							<Accordion.Body>
								<Form.Group className="mb-3">
									...
									<Form.Control
										as="select"
										onChange={e =>
											setSelectedSuggestionWithPosition(
